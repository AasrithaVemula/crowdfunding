...
const AdminOnly = ({ component: Component, path, loggedIn, isAdmin, exact }) => (
  <Route
    path={path}
    exact={exact}
    render={props =>
      loggedIn && isAdmin ? <Component {...props} /> : <Redirect to="/" />
    }
  />
);
...
const mapStateToProps = state => ({
  ...
  isAdmin: Boolean(state.entities.users[state.session.id]?.isAdmin)
});

export const AuthRoute = withRouter(connect(mapStateToProps)(Auth));
export const ProtectedRoute = withRouter(connect(mapStateToProps)(Protected));
export const AdminRoute = withRouter(connect(mapStateToProps)(AdminOnly));
